...
\- \*\*Preprocessing:\*\* lowercased, tokenized with Keras `Tokenizer`, converted into n-gram input sequences, padded to a uniform length (`max\_sequence\_len = 14`)
...
| Recurrent 1 | LSTM(150, return\_sequences=True) | GRU(150, return\_sequences=True) |
...
\*\*Training config:\*\* Adam optimizer, categorical crossentropy loss, 50 epochs (capped by early stopping on `val\_loss`, patience=3), 80/20 train-test split.
...
!\[LSTM vs GRU Comparison](lstm\_vs\_gru\_comparison.png)
...
├── lstm\_vs\_gru\_comparison.png    # Validation loss/accuracy comparison plot

├── next\_word\_lstm.keras          # Saved LSTM model
